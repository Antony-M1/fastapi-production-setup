...
```
This will start Gunicorn or Uvicorn on the same interface that the Fastapi development server was runnig on. We can go back and test the app again in your browser

## Creating systemd Socket and Service Files
We have tested that Gunicorn can interact with our application, but we should now implement a more robust way of starting and stopping the application server.
